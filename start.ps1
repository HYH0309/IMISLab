...
if (-not $SkipRedis) {
    Write-Host "1️⃣ Starting Redis Server..." -ForegroundColor Yellow
    ...
    Write-Host "1️⃣ Skipping Redis startup (as requested)" -ForegroundColor Yellow
}

Write-Host ""

# Start Backend
Write-Host "2️⃣ Starting Backend Service..." -ForegroundColor Yellow
...
if (Test-Path $backendBinary) {
    ...
    Write-Host "   ⚠️  Backend binary not found, checking for Go..." -ForegroundColor Yellow
...
# Start Frontend
Write-Host "3️⃣ Starting Frontend Development Server..." -ForegroundColor Yellow
